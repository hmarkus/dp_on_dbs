...
where small treewidth is one of the most cited\footnote{On October 17, 2019, Google Scholar shows 19,200 results on treewidth.} 
...
settings~\cite{AbseherMusliuWoltran17a} \todo{actually, we require the normalize-cli branch...}, which finds TDs extremely quick
...
\begin{itemize}
	...
	%\item 
	\makebox[8.25em][l]{}{\ttfamily $\tab{1}$.$\cid{a_1}$ - $\ldots$ - ($\Sigma_{i=1}^\ell \Card{\chi(t_i) \cap\{a_1\}}$ - 1) * $\tab{1}$.$\cid{a_\ell}$}
...
PostgreSQL version, etc.\todo{add}

\paragraph{Other Benchmarked Solvers.}
...
\begin{itemize}
	...
	\item approximate memory used by db for one of the largest problems solved
	\item Discussion section: we don't create/use any indices... Meaningful/useful B*Tree indices hard to create. Exploration of Bitmap indices (Oracle Enterprise feature) would be interesting.
	\item effect of parallel queries that are not availbale in PostgreSQL 9.5, added to 9.6 and improved in every version since that, i.e., versions 10, 11 as well as 12.
\end{itemize}
