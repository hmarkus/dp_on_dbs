...
\begin{itemize}
	...
	\item\makebox[8.25em][l]{\tuplecolor{\inputPredColor}{$\#\mathsf{extProj}\#$}:}{\ttfamily \tab{1}.cnt $\cdot \ldots \cdot \tab{\ell}$.cnt AS cnt }
\end{itemize}
...
Problem $\VC$ for node~$t$ with child nodes~$t_1, \ldots, t_\ell$, where~$E(G_t)=\{\{u_1,v_1\},\ldots, \{u_n,v_n\}\}$ and~$\chi(t)=\{a_1,\ldots,a_{k}\}$ can be specified as follows.
...
\begin{itemize}
	...
	\item\makebox[8.25em][l]{\tuplecolor{\inputPredColor}{$\#\mathsf{extProj}\#$}:}{\ttfamily $\tab{1}$.card + $\ldots$ + $\tab{\ell}$.card - $\Sigma_{}$ - $\ldots$ - $\Sigma_{}$}
\end{itemize}
