...
\section{Relating Dynamic Programming and~Relational~Algebra}
...
%\centering
  ...
  \caption{Table algorithm~$\algo{S}(t,\chi(t),\varphi_t,\langle \tab{1}, \ldots, \tab{\ell}\rangle)$ for \cSAT~\cite{SamerSzeider10b} using nice TDs.}
  \label{alg:prim}
  \algorithmfootnote{%
...
%\centering
  \KwData{Node~$t$, bag $\chi(t)$, clauses~$\varphi_t$,
    sequence $\langle \tab{1},\ldots \tab{\ell}\rangle$ of child tables.{~\bf Out:} Table $\tab{t}.\hspace{-5em}$} \lIf(\hspace{-1em})
   %
  {$\type(t) = \leaf$}{\label{line:leaf}%
    $\tab{t} \eqdef \tuplecolor{\inputPredColor}{\{ \{{(cnt, 1)}\} \}}$%
  }%
  \uElseIf{$\type(t) = \intr$, and
    $a\hspace{-0.1em}\in\hspace{-0.1em}\chi(t)$ is introduced}{ \label{line:intr}%
    $\tab{t}\eqdef \tab{1} \bowtie_{\tuplecolor{\inputPredColor}{\varphi_t}} \tuplecolor{\inputPredColor}{\{\{(\cid{a},0)\},\{(\cid{a},1)\}\}}$
    \hspace{-5em}\vspace{-0.05em}
       % 
  }\vspace{-0.05em}%
  % \alpha \setminus \{e \mapsto 0, e \mapsto 1\}
  \uElseIf{$\type(t) = \rem$, and $a \not\in \chi(t)$ is removed}{\label{line:rem}%
    $\tab{t} \eqdef {_{\chi(t)}G}_{\tuplecolor{\inputPredColor}{\text{SUM}(cnt)}}(\Pi_{{\attr(\tab{1})\setminus\{\cid{a}}\}}\tab{1})$ \hspace{-5em} \vspace{-0.1em}
     %
  } %
  \uElseIf{$\type(t) = \join$}{\label{line:join}%
    \makebox[3.3cm][l]{$\tab{t} \eqdef \Pi_{\attr(\tab{1})}[\dot\Pi_{\tuplecolor{\inputPredColor}{\{cnt \leftarrow cnt \cdot cnt'\}}}(\tab{1} \bowtie_{\bigwedge_{a\in\chi(t)}\cid{a}\approx \cid{a}'} \rho_{\hspace{-1.2em}\bigcup\limits_{a\in \attr(\tab{2})}\hspace{-1.2em}\{\cid{a}\mapsto \cid{a}'\}}\tab{2})]$}  %\{ \langle
  ...
  %\Return $\tab{t}$ \vspace{-0.25em}
  \caption{Table algorithm~$\algo{S_{RAlg}}(t,\chi(t),\varphi_t,\langle \tab{1}, \ldots, \tab{\ell}\rangle)$ for \cSAT using~nice~TDs.}
  \label{alg:primdb}
   %\algorithmfootnote{%
   % \renewcommand{\eqdef}{{\ensuremath{\,\mathrel{\mathop:}=}}}
   % \label{foot:sigma}\label{foot:abrevtwo}
...
Actually, the core of this architecture is focused on the table algorithm~$\algo{A_{RAlg}}$, shown in blue. 
...
on certain normalization or forms of TDs, i.e., whether it is nice or not.
...
\begin{algorithm}[t]
%\centering
  \KwData{Node~$t$, bag $\chi(t)$, problem~$\mathcal{I}_t$,
    sequence $\langle \tab{1},\ldots \tab{\ell}\rangle$ of child tables.{~\bf Out:} Table $\tab{t}.\hspace{-5em}$} \lIf(\hspace{-1em})
   %
  {$\type(t) = \leaf$}{\label{line:leaf}%
    $\tab{t} \eqdef \tuplecolor{\inputPredColor}{\#\epsilon\mathsf{Tab}\#}$%
  }%
  \uElseIf{$\type(t) = \intr$, and
    $a\hspace{-0.1em}\in\hspace{-0.1em}\chi(t)$ is introduced}{ \label{line:intr}%
    $\tab{t}\eqdef \tab{1} \bowtie_{\tuplecolor{\inputPredColor}{\#\mathsf{localProbFilter}\#}} \tuplecolor{\inputPredColor}{\#\mathsf{intrTab}\#}$
    \hspace{-5em}\vspace{-0.05em}
       % 
  }\vspace{-0.05em}%
  % \alpha \setminus \{e \mapsto 0, e \mapsto 1\}
  \uElseIf{$\type(t) = \rem$, and $a \not\in \chi(t)$ is removed}{\label{line:rem}%
    $\tab{t} \eqdef {_{\chi(t)}G}_{\tuplecolor{\inputPredColor}{\#\mathsf{aggrExp}\#}}(\Pi_{\attr(\tab{1})\setminus\{\cid{a}\}}\tab{1})$ \hspace{-5em} \vspace{-0.1em}
     %
  } %
  \uElseIf{$\type(t) = \join$}{\label{line:join}%
    \makebox[3.3cm][l]{$\tab{t} \eqdef \Pi_{\attr(\tab{1})}[\dot\Pi_{\tuplecolor{\inputPredColor}{\#\mathsf{extProj}\#}}(\tab{1} \bowtie_{\bigwedge_{a\in\chi(t)}\cid{a}\approx \cid{a}'} \rho_{\hspace{-1.2em}\bigcup\limits_{a\in \attr(\tab{2})}\hspace{-1.2em}\{\cid{a}\mapsto \cid{a}'\}}\tab{2})]$}  %\{ \langle
      %\tuplecolor{\inputPredColor}{I},
      %\tuplecolor{\statePredColor}{c_1 \cdot c_2}
      %\rangle
      %$}%$|\;\langle \tuplecolor{\inputPredColor}{I},
  ...
  %\Return $\tab{t}$ \vspace{-0.25em}
  \caption{Template of~$\algo{A_{RAlg}}(t,\chi(t),\mathcal{I}_t,\langle \tab{1}, \ldots, \tab{\ell}\rangle)$ for instance~$\mathcal{I}$ of $\mathcal{P}$.}
  \label{alg:template}
\end{algorithm}%
...
The system is open-source, written in Python 3 and uses PostgreSQL as database.
