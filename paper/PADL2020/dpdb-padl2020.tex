...
Apart from this trade-off, we tried to outsource the task of joining tables to the database as much as possible, since the performance of database systems highly depend on query plan optimization. % and should know best, how to join.
...
This query consists of serveral parts. 
First of all, 
...
\paragraph*{Logging and Debugging.} Currently, we currently have two versions of the \dpdb system implemented. 
...
Further, since for each table~$\tab{t}$, exactly one SQL statement is executed for filling this table, we also have a dedicated view of the SQL SELECT statement, whose result was inserted.
...
\paragraph*{Configurations and Extensions.} Within one node~$t$ the computation of~$\tab{t}$ uses child tables of~$t$.
...
The system \dpdb allows for \emph{easy protyping}
