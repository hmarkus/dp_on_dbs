...
Finally, for a node~$t$ with~$\type(t)=\join$, in Line~\ref{line:join} we use again $\theta$-joins (and extended projection for maintaining counters), which allows us later to leverage advanced database technology. %of the last decades.
...
\paragraph*{Logging, Debugging and Extensions.} We currently have two versions of the \dpdb system implemented. 
