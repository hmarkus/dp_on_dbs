...
techniques from parameterized complexity~\cite{FlumGrohe06,DowneyFellows13,Niedermeier06,CyganEtAl15}. 
...
%Treewidth 
and is one of the most cited\footnote{On October 17, 2019, Google Scholar shows 19,200 results on treewidth. Treewidth was also subject of recent competitions~\cite{DellHusfeldtJansen17a,DellKomusiewiczTalmon18a}.} 
combinatorial invariants studied in parameterized complexity.
...
Table algorithm~$\algo{S}$ as presented in Listing~\ref{alg:prim} shows all the cases that are needed to solve~\cSAT by means of DP over nice TDs.
...
For join nodes~$t$, counters of common assignments in the child tables are multiplied, as shown in Line~7.
...
uses extended projection as in Line~\ref{line:join2}. % compared to Listing~\ref{alg:primdb}.
...
handling \emph{problem-specific} implementations of the placeholders of Listing~\ref{alg:template}, where the system following this architecture is responsible for interleaving and overlapping these cases within a node~$t$.
...
\begin{figure}[t]
  ...
  %
  \centering
  \resizebox{.75\columnwidth}{!}{\includegraphics{plot_pmc.pdf}}
  \resizebox{.75\columnwidth}{!}{\includegraphics{plot_pmc_35_enlarged.pdf}}
  %
  %
  %
  %
  %
  %
  \caption{Runtime for the top 15 solvers over \emph{all}~\cSAT instances (top) / \emph{only} instances up to treewidth upper bound 35 (bottom). %
    %
    %
    ...
    \begin{tabular}{{l|lH||rrrrrrrr||r||rHr}}
     ...
     %
      ...
      &c2d & \textbf{0} & 1124 & 31 & \textbf{10} & 3 & 3 & 10 & 20 & 0 & 1181 & 84.41 & 6 \\
       & \dpdb & 0 & 1155 & 19 & 5 & 2 & 0 & 0 & 2 & 1 & 1181 & 100.40 \\
...
%
in two cactus-like plots, where the x-axis refers to the number 
of instances and the y-axis depicts the runtime sorted in ascending order for each solver individually.
At the top, we provide an overview over all the benchmarked~\cSAT instances.
Below, a more detailed picture is drawn, where only instances up to an upper bound\footnote{These upper bounds were obtained via decomposer htd in at most two seconds.} of treewidth 35 are considered.
...
If one considers only instances with TDs up to width 35 as in Figure~\ref{fig:runtime} (bottom), \dpdb is able to solve slightly more instances than c2d as well as countAntom.
Surprisingly, our system shows a different runtime behavior than the other solvers. 
We believe that reason lies in an initial overhead caused by the creation of the tables that seems to depend on the number of nodes of the used TD.
There, I/O operations between main memory and hard disk seem to kick in.
%
%
