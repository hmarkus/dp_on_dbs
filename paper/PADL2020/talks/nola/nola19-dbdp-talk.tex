...
\begin{frame}{Research Question \uncover<6>{\& Approach}}
  \medskip
  \alert{Can we merge the two approaches? (and make it reasonable to use?)}\\[0.5em]
  \begin{itemize}
  \item[Idea 1:]<2-> Provide a programming framework.\\
    Already done.\\
    $\Rightarrow$ Sharp (C++) \bcite{Morak'11} and Jatatosk (Java) \bcite{Bannach\&Berndt'18}\\
    \uncover<3->{\hspace{1em}\xmark~still hard to use \hspace{4.7em} \xmark~performs nice, but far from competitive}
    \medskip
  \item[Idea 2:]<4-> Use a logic based language to specify the DP\\
    Already done.\\
    $\Rightarrow$ D-FLAT (use ASP for spec and take an ASP solver for
    computation) \bcite{Bliem\&Morak\&W'12}\\
    \uncover<5->{\hspace{1em}\xmark~still fairly hard to use and performs mostly not very well}
    \color{red}
  \end{itemize}
  \uncover<6->{%
...
\begin{frame}<1,2,10>[noframenumbering]{Treewidth}\label{lbl:motivation}
  \only<1>{
  \bigskip
  \begin{columns}
...
\begin{frame}[noframenumbering]{}
  ...
  \bigskip
  \begin{center}
    \alert{\Large How to ``use'' tree decompositions for \#SAT?}\\
  \end{center}
\end{frame}
 ...
 where $\ell_i$ either $x$ or $\neg x$
  ...
  \uncover<16->{%
    \begin{block}{Model Counting (\#SAT/Number SAT)}
      \begin{itemize}
      \item Number of satisfying truth assignments to~$F$.
      \end{itemize}
    \end{block}
    %
  }%
          ...
          \begin{algorithm}[H]
            ...
            \vspace{-1em} \uElseIf{$\intr$ and $a\in V_t$ new}{ %
              \Return $\sigma_{row \models F_t}\left( T_\downarrow
                \times \left(\begin{matrix} a \\\hline 0 \\
