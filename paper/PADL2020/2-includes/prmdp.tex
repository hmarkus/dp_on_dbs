...
%\centering
  \KwData{Node~$t$, bag $\chi(t)$, clauses~$\varphi_t$,
    sequence $\langle \tab{1},\ldots \tab{\ell}\rangle$ of child tables.{~\bf Out:} Tab $\tab{t}.\hspace{-5em}$} \lIf(\hspace{-1em})
   %
  {$\type(t) = \leaf$}{%
  ...
  \uElseIf{$\type(t) = \intr$, and
    ...
    $\tab{t}\eqdef \rho_{\varphi_t}(\tab{1}\times\{\{(a,0)\},\{(a,1)\}\})$
    \hspace{-5em}\vspace{-0.05em}
       % 
  }\vspace{-0.05em}%
  % \alpha \setminus \{e \mapsto 0, e \mapsto 1\}
  \uElseIf{$\type(t) = \rem$, and $a \not\in \chi(t)$ is removed}{%
    $\tab{t} \eqdef {_{\chi(t)}G}_{\sum(cnt)}(\Pi_{\chi(t)\cup\{cnt\}}(\tab{1}))$ \hspace{-5em} \vspace{-0.1em}
     %
  } %
  \uElseIf{$\type(t) = \join$}{%
    \makebox[3.3cm][l]{\hspace{-1em}$\tab{t} \eqdef \Pi_{\MAIR{\chi(t)}{cnt}}[\Pi_{cnt \leftarrow cnt_1 \cdot cnt'}(\rho_{\{cup \mapsto cup_1\}}\tab{1} \bowtie_{\bigwedge_{a\in\chi(t)}a\approx a'} \rho_{\{a\mapsto a' \mid a\in \MAIR{\chi(t)}{cnt}\}}(\tab{2}))]$}  %\{ \langle
  ...
  \caption{Table algorithm~$\algo{S_{Alg}}(t,\chi(t),\varphi_t,\langle \tab{1}, \ldots, \tab{\ell}\rangle)$ for \cSAT using nice TDs.}
  \label{alg:prim}
  %\algorithmfootnote{%
