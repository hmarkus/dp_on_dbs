...
class Sat(Problem):
    ...
    def create_assignment_view(self,node):
        ...
        q += (
        """\tFROM %s
        """) % ",".join(set([Sat.var2tab(n,from_tdn[n]) for n in bag] + ["td_node_{0} t{0}".format(n.id) for n in node.children]))

        if needs_join:
            q += (
            """\tWHERE %s
            """) % " AND ".join(filter(None,[Sat.var2join(n,from_tdn[n]) for n in bag]))
